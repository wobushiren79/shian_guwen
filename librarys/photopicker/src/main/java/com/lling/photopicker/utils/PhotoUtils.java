...
public class PhotoUtils {
    ...
    public static Map<String, PhotoFloder> getPhotos(Context context) {
        ...
        if(floderMap.size()==1){
        	Uri imageUri2 = MediaStore.Images.Media.INTERNAL_CONTENT_URI;
            ContentResolver mContentResolver2 = context.getContentResolver();

            // 只查询jpeg和png的图片
            Cursor mCursor2 = mContentResolver2.query(imageUri2, null,
                    MediaStore.Images.Media.MIME_TYPE + " in(?, ?)",
                    new String[] { "image/jpeg", "image/png" },
                    MediaStore.Images.Media.DATE_MODIFIED + " desc");

            int pathIndex2 = mCursor2
                    .getColumnIndex(MediaStore.Images.Media.DATA);

            mCursor2.moveToFirst();
            while (mCursor2.moveToNext()) {
                // 获取图片的路径
                String path = mCursor2.getString(pathIndex2);
