...
public class ImageLoader {
    ...
    public static synchronized ImageLoader getInstance(Context applicationContext) {
        if(mInstance == null) {
            mInstance = new ImageLoader(applicationContext);
        }
        return mInstance;
    }
     ...
     * 初始化磁盘缓存
     */
    private void initDiskCache(Context applicationContext) {
        try {
            File cacheDir = OtherUtils.getDiskCacheDir(applicationContext, "images");
            if (!cacheDir.exists()) {
                cacheDir.mkdirs();
            }
            mDiskLruCache = DiskLruCache.open(cacheDir, OtherUtils.getAppVersion(applicationContext),
                    1, 15 * 1024 * 1024);
        } catch (IOException e) {
            e.printStackTrace();
        }
