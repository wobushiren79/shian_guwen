...
public class DateSelectorWheelView extends RelativeLayout implements
	...
	private StrericWheelAdapter minsAdapter;
	...
	private void initLayout(Context context) {
		...
		tvMin = (TextView) findViewById(R.id.tv_date_time_min);
		...
		wvMin = (WheelView) findViewById(R.id.wv_date_of_min);
	...
	private void setData() {
		...
		minsAdapter = new StrericWheelAdapter(mins);
		...
		wvMin.setAdapter(minsAdapter);
		wvMin.setCurrentItem(getTodayMin());
		wvMin.setCyclic(true);
	...
	private int getTodayHour() {
		...
		int position = 0;
		String today = getToday();
		String day = today.substring(11, 13);
		day = day + " 时";
		for (int i = 0; i < hours.length; i++) {
			if (day.equals(hours[i])) {
				position = i;
				break;
			}
		}
	...
	private int getTodayMin() {
		// 2015年12月01日
		int position = 0;
		String today = getToday();
		String day = today.substring(14, 16);
		day = day + " 分";
		for (int i = 0; i < mins.length; i++) {
			if (day.equals(mins[i])) {
				position = i;
				break;
			}
		}
	...
	public void onChanged(WheelView wheel, int oldValue, int newValue) {
		...
		} else if (id == R.id.wv_date_of_hour) {
			tvHour.setText(DateUtils.splitDateString(
					wvHour.getCurrentItemValue()).trim());
		} else if (id == R.id.wv_date_of_min) {
			tvMin.setText(DateUtils
					.splitDateString(wvMin.getCurrentItemValue()).trim());
		}
	}
